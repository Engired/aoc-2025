...
///////////////////////////////////////
	...
	{		
		...
		// if there's only one circuit
		if(n_cir == 1)
		{
			// and every id is included
			if(circ[0].n_ids >= n_boxes)
			{
				ret = boxes[dist_sort[i].id].x * boxes[dist_sort[i].place].x;
				break;
			}
		}
	}
